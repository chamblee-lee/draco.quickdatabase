...
namespace Draco.DB.ORM.SqlServerClient
{
    ...
    /// 自增
    /// </summary>
    public class PKGeneratorSQLServerAuto : IPKGenerator
    {
        /// <summary>
        /// 是否可复用
        /// </summary>
    ...
    /// 自定义序列
    /// </summary>
    public class PKGeneratorSQLServerORMSequence : IDBReferencePKGenerator
    {
        ...
        private void InitTable(string TableName)
        {
            try
            {
                string SQL = "select * from   sysobjects where   name= '{0} ' and  xtype= 'U '";
                SQL = String.Format(SQL, TableName);
                DataSet ds = m_IDBHandler.ExecuteQuery(SQL);
                if (ds == null || ds.Tables[0].Rows.Count == 0)
                {
                    //表不存在
                    string CreateSQL = "CREATE TABLE [dbo].[ORM_SEQUENCE]("+
	                "[TableName] [varchar](256) Primary key NOT NULL,"+
